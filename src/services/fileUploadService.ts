...
class FileUploadService {
  ...
  private async uploadToNetlify(file: File, onProgress?: (progress: UploadProgress) => void): Promise<UploadResult> {
    try {
  ...
  private async uploadAsBase64(file: File, onProgress?: (progress: UploadProgress) => void): Promise<UploadResult> {
  ...
  private async uploadWithCompression(file: File, onProgress?: (progress: UploadProgress) => void): Promise<UploadResult> {
    try {
      onProgress?.({ progress: 0, status: 'uploading' })
  ...
  private async aggressiveCompressFile(file: File): Promise<File> {
    return new Promise((resolve, reject) => {
  ...
  validateFileForI2I(file: File): boolean {
    const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp']
    return allowedTypes.includes(file.type)
  }

  validateFileForV2V(file: File): boolean {
