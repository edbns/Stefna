...
// Legacy function for backward compatibility - now uses new preset system
export async function onTimeMachineClick(option: string, file?: File, sourceUrl?: string) {
  try {
    // Import the new integration handler
    const { onTimeMachineClick: newHandler } = await import('../utils/presets/integration');
    return await newHandler(option, file, sourceUrl);
  } catch (error) {
    console.error('Failed to load new time machine handler:', error);
    // Fallback to showing error
    window.dispatchEvent(new CustomEvent('generation-error', { 
      detail: { message: `${option} is not available`, timestamp: Date.now() } 
    }));
  }
}
