...
export const PARALLEL_SELF_PRESETS: ParallelSelfPreset[] = [
  ...
  // 🎨 Colorcore
  {
    ...
    label: 'Colorcore',
    prompt: 'Create a single photoreal square image (aspect ratio 1:1) that contains four vertically oriented portrait frames, arranged in a 2x2 grid (two at the top, two at the bottom). Each portrait frame must follow a 6:19 vertical aspect ratio inside the square layout, showing the same subject(s) — person, couple, or group — in different poses, outfits, and expressions. All people in the input image must appear in every frame. No cropping or solo close-ups. Keep group integrity consistent. Each of the four portrait frames must be evenly spaced with clean white borders between each frame, and a thick white outer margin around the entire grid. The four frames are tall, vertical portraits, arranged like this: 🟨🟨 🟨🟨 FRAMING: Upper body only, from the waist up. Show head, shoulders, torso, and arms only. No legs or full body shots. Use a solid butter yellow background in all 4 frames — bright, vibrant, and clean. No patterns, textures, props, or gradient. All outfits and accessories must follow a yellow theme, but vary between the frames. Include a mix of fashion aesthetics: casual, sexy, streetwear, modest, etc. IMPORTANT GENDER-SPECIFIC STYLING: Women wear ONLY feminine clothing: pastel dresses, knit tops, crop hoodies, fashion lingerie, casual tees. Men wear ONLY masculine clothing: yellow oversized tees, bomber jackets, sleeveless vests, long tunics. Never dress men in women\'s clothing or women in men\'s clothing. Each person keeps their gender-appropriate styling across all 4 frames. Couples or groups should show slight changes per frame — jacket off/on, different pose or accessories — but remain harmonious and gender-appropriate. Poses per frame (each frame must show a different one): Finger heart, Peace sign near the eye, Chin resting on hand, Cheek squish or soft smile. Lighting: bright studio style — high clarity, no shadows, clean beauty light across all 4 frames. Final image must look like a professional 2×2 collage with a unified aesthetic: strong yellow pop, styling variation, expressive energy, and clean white grid borders.',
    negative_prompt: 'cartoon, anime, multiple people in one frame, overlapping, dark lighting, shadows, patterns, textures, props, gradients, full body, legs, feet, standing pose, walking, cross-dressing, men in dresses, men in feminine clothing, women in masculine suits, gender swap',
    strength: 0.6,
    model: 'fal-ai/nano-banana/edit',
    mode: 'i2i',
    input: 'image',
