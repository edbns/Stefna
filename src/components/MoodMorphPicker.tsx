...
import { MOODMORPH_PRESETS, MoodMorphPreset } from '../presets/moodmorph';

interface MoodMorphPickerProps {
  presets?: MoodMorphPreset[];
  value?: string;
  onChange?: (id: string) => void;
  disabled?: boolean;
}

export function MoodMorphPicker({
  presets = MOODMORPH_PRESETS,
  value,
  onChange,
  disabled = false,
}: MoodMorphPickerProps) {
  return (
    <div className="space-y-1">
      ...
      {/* MoodMorph preset options - matches presets exactly */}
      {presets.map((preset) => (
        <button
          key={preset.id}
