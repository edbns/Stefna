...
const TokenCounter: React.FC<TokenCounterProps> = ({ 
  ...
  const [previousTokens, setPreviousTokens] = useState<number>(0)
  ...
  const getTokenColor = () => {
    if (remainingTokens <= 2) return 'text-red-400'
    if (remainingTokens <= 5) return 'text-yellow-400'
  ...
  if (isLoading) {
    return (
      <div className={`w-8 h-8 bg-white/10 rounded-full flex items-center justify-center animate-pulse ${className}`}>
  ...
  return (
    <div 
      className={`w-8 h-8 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-white/20 transition-all duration-300 border border-white/20 cursor-pointer ${getAnimationClass()} ${className}`} 
      title={`${remainingTokens} tokens remaining`}
    >
      <span className={`text-sm font-medium ${getTokenColor()}`}>
        {remainingTokens}
      </span>
    </div>
  )
