...
async function processStoryJob(job, job_id) {
    ...
    `.replace(/\s+/g, ' ').trim();
        ...
        const upload = await cloudinary.uploader.upload(outputFile, {
            resource_type: 'video',
...
export const handler = async (event) => {
    ...
    try {
        ...
        if (V2V_WEBHOOK_SECRET)
            payload.webhook_secret = V2V_WEBHOOK_SECRET;
