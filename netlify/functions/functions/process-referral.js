...
export const handler = async (event) => {
    try {
        ...
        if (!referrerEmail)
            return json(400, { ok: false, error: "MISSING_REFERRER_EMAIL" });
        const db = getDb();
        ...
        await db.query(`INSERT INTO referral_signups(referrer_user_id, new_user_id, referrer_email, new_user_email)
       VALUES ($1::uuid,$2::uuid,$3,$4)
       ON CONFLICT (new_user_id) DO NOTHING`, [referrerId, newUserId, referrerEmail, newUserEmail ?? null]);
        // Only award if this is the first time (check ledger for existing referral grants)
        const already = await db.query(`SELECT 1 FROM credits_ledger
       WHERE user_id=$1::uuid AND status='granted' AND meta->>'reason'='referral_referrer'
       AND meta->>'new_user_id'=$2 LIMIT 1`, [referrerId, String(newUserId)]);
        if (!already.rowCount) {
            ...
            // Grant to referrer
            await db.query("SELECT app.grant_credits($1::uuid,$2::int,'referral.referrer',jsonb_build_object('reason','referral_referrer','new_user_id',$3))", [referrerId, refBonus, String(newUserId)]);
            // Grant to new user
            await db.query("SELECT app.grant_credits($1::uuid,$2::int,'referral.new',jsonb_build_object('reason','referral_new','referrer_user_id',$3))", [newUserId, newBonus, String(referrerId)]);
        }
