...
export const handler: Handler = async (event) => {
	...
	try {
		...
		// Build tags and set via uploader.explicit (avoid api.add_tag)
		const makePublic = !!(shareNow || shareToFeed);
		const tags: string[] = ['stefna', 'type:output'];
		if (uid && uid !== 'anonymous') tags.push(`user:${uid}`);
		if (makePublic) tags.push('public');

		await cloudinary.uploader.explicit(upload.public_id, {
			type: 'upload',
			resource_type: commonOptions.resource_type,
			tags,
			context: { user_id: uid },
		});
