...
export const handler = async (event) => {
    try {
        ...
        // Poll the vendor with the I2V model
        ...
        const status = String(data.status || data.state || (data.video_url || data.content?.url ? 'completed' : '')).toLowerCase();
        ...
        // Persist to Cloudinary as VIDEO (I2V generates videos)
        const uploadPayload = {
            url: resultUrl,
            resource_type: 'video',
