...
export const handler = async (event) => {
    ...
    `);
        ...
        const { rows: balanceRows } = await db.query("SELECT balance FROM app.user_credits WHERE user_id = $1::uuid", [userId]);
        const currentBalance = balanceRows[0]?.balance || 0;
        // Test 4: Check daily usage
        console.log('🧪 Test 4: Checking daily usage...');
        const { rows: dailyRows } = await db.query("SELECT app.allow_today_simple($1::uuid, $2::int) AS allowed", [userId, parseInt(config.image_cost) || 2]);
