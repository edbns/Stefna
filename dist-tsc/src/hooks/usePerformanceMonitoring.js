...
export function usePerformanceMonitoring(config = {}) {
    const { enableCoreWebVitals = false, // Temporarily disabled
    ...
    const trackImageLoad = (startTime, endTime, imageUrl) => {
        if (!shouldMonitor.current)
            return;
    ...
    const trackApiCall = (endpoint, startTime, endTime) => {
        if (!shouldMonitor.current)
            return;
...
export function usePerformantFetch() {
    ...
    const performantFetch = async (url, options) => {
