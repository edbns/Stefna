...
import { preventDuplicateOperation, networkGuardRails } from '../utils/guardRails';
...
const isFileLike = (x) => typeof x === "object" && x !== null && "size" in x && "type" in x;
let uiState = {
    busy: false,
    currentRunId: null,
    activeRuns: new Map()
...
export const uiStore = {
    ...
    abortStaleRuns: (maxAgeMs = 300000) => {
        const now = Date.now();
...
export async function runGeneration(buildJob) {
...
async function callAimlApi(job, options) {
    try {
...
export function subscribeToUIState(callback) {
    const handler = (event) => callback(event.detail);
    window.addEventListener('ui-state-change', handler);
    return () => window.removeEventListener('ui-state-change', handler);
}
...
async function onGenerationComplete(result, job) {
