...
class AIGenerationService {
    ...
    constructor() {
        Object.defineProperty(this, "generationQueue", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    async generateContent(request) {
        ...
        try {
            ...
            if (request.imageFile) {
                ...
                const uploadResult = await fileUploadService.uploadFile(request.imageFile, (progress) => {
                    this.updateGenerationStatus(generationId, {
            ...
            if (request.videoFile) {
                ...
                const uploadResult = await fileUploadService.uploadFile(request.videoFile, (progress) => {
                    this.updateGenerationStatus(generationId, {
                        uploadProgress: progress,
                        progress: 10 + (progress.progress * 0.3) // Upload is 30% of total progress
                    });
    ...
    async saveToHistory(result, userId) {
        try {
    ...
    clearGenerationStatus(generationId) {
    ...
    selectBestModel(request, imageUrl, videoUrl) {
    ...
    // Call real AIML API
    async callAimlApi(request, generationId, modelId, imageUrl, videoUrl) {
        try {
