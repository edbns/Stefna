...
export function validatePresets(reg) {
    const errs = [];
    for (const [id, p] of Object.entries(reg)) {
        if (!p.id || id !== p.id)
            errs.push(`[${id}] id must exist and match key`);
        if (!p.label)
            errs.push(`[${id}] label required`);
        if (!p.prompt)
            errs.push(`[${id}] prompt required`);
        if (!ALLOWED[p.mode])
            errs.push(`[${id}] invalid mode ${p.mode}`);
        else {
            const rule = ALLOWED[p.mode];
            if (!rule.input.includes(p.input))
                errs.push(`[${id}] input "${p.input}" not allowed for mode "${p.mode}"`);
...
export function validateOptions(groups, reg) {
    const errs = [];
    const keys = new Set(Object.keys(reg));
    for (const [group, entries] of Object.entries(groups)) {
        for (const [key, ref] of Object.entries(entries ?? {})) {
            if (!keys.has(String(ref.use)))
                errs.push(`[${group}/${key}] references missing preset "${String(ref.use)}"`);
    ...
    if (errs.length)
        console.warn(`validateOptions: ${errs.length} issue(s)`, errs);
    else
        console.info('✅ validateOptions: OK');
    return errs;
}
...
export function validateUIConfiguration() {
    ...
    if (errs.length)
        console.warn(`validateUIConfiguration: ${errs.length} issue(s)`, errs);
    else
        console.info('✅ validateUIConfiguration: OK');
    return errs;
}
// New function that only validates when presets are ready
export function validateUIConfigurationWhenReady() {
