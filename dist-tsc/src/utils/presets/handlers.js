...
export async function runPreset(preset, srcOverride, metadata) {
    const runId = crypto.randomUUID();
    console.info(`🎯 [${runId}] Running preset:`, preset.label);
    try {
        // 1) Source: prefer override (fresh Cloudinary URL), else global store
        let src = preset.requiresSource ? (srcOverride ?? (getCurrentSourceUrl() || undefined)) : undefined;
...
export async function onPresetClick(presetId, srcOverride) {
    try {
...
export async function onOptionClick(group, key, srcOverride) {
    try {
...
async function generatePreset(preset, srcOverride, metadata) {
    if (isGenerating) {
...
export function canRunPreset(srcOverride) {
...
export async function withTelemetry(ctx, fn) {
