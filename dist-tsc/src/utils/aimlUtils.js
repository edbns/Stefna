...
export function buildEditPayload({ prompt, sourceUrl, steps, strength, resourceType }) {
    const common = {
        prompt,
 ...
 */
export function pickResultUrl(body) {
 ...
 */
export async function ensureRemoteUrl(previewUrl, file) {
    if (previewUrl && !previewUrl.startsWith('blob:'))
        return previewUrl;
    if (!file)
        throw new Error('Invalid asset URL and no file to upload');
