...
export const useIntentQueue = create((set, get) => ({
    pending: null,
    sourceUrl: null,
    isUploading: false,
    isGenerating: false,
...
export function hasHttpsUrl(url) {
    return typeof url === 'string' && url.startsWith('https://');
}
/** Public API used by ALL buttons - single orchestration point */
export async function ensureSourceThenRun(intent) {
...
export function openHiddenUploader() {
...
export function mapApiErrorToUserMessage(status, message) {
    switch (status) {
        case 400:
        case 422:
