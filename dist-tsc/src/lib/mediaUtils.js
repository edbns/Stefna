 ...
 */
export function toFinalUrl(publicId, resourceType) {
 ...
 */
export function getOptimizedUrl(publicId, resourceType, options) {
    const cloudName = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME;
    if (!cloudName)
        return toFinalUrl(publicId, resourceType);
    const base = `https://res.cloudinary.com/${cloudName}`;
...
export function getVideoThumbnail(publicId, options) {
    const cloudName = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME;
    if (!cloudName)
        return toFinalUrl(publicId, "image");
 ...
 */
export function mapFeedItemUrls(item) {
