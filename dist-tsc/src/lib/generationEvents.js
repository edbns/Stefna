 ...
 */
export const generationStart = (detail) => {
 ...
 */
export const generationDone = (detail) => {
...
export const useGenerationEvents = () => {
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener('generation:start', handleStart);
            window.addEventListener('generation:done', handleDone);
        }
        return () => {
            listeners.delete(updateState);
            if (typeof window !== 'undefined') {
                window.removeEventListener('generation:start', handleStart);
 ...
 */
export const withGenerationCompletion = async (operation, kind = 'image') => {
 ...
 */
export const withGenerationTimeout = (timeoutMs = 90000) => {
