// src/lib/credits.ts
export async function runGeneration({ token, action = "image.gen", baseUrl = "/.netlify/functions", runJob, // (requestId) => Promise<{ ok: boolean }>
 }) {
    ...
    if (!r.ok)
        throw new Error((await r.json()).error ?? "Reserve failed");
    // run AIML job
    let success = false;
    try {
    ...
    if (!f.ok)
        console.warn("Finalize non-OK", await f.text());
    if (!success)
