1-add delete and download icon back to the ALl tab media
2-change the icon design of the user profile to something that matches the other icons and don't delete the progress implementation around it
3-change social media icons to something else unified now they don't look like the other icons 
4-share, like and remix button on the media card on the home page are not implemented (system is already in place just implement them)
5-make sure floating notification is not showing title and text under it, make it show only title , make it compact and move it little down under the nav bar so it doesn't hide it.
6-user profile All  media tab doesn't show masonry layout all aspect ration and crops them check screenshot
7-home scree doesn't show masonry layout all aspect ration too. Also add the function of media viewer to the home page media card, its already exist.
8-make sure the media viewer extract the real prompt from the media and its not showing AI generated content only -- Stefna Unified Schema Migration
