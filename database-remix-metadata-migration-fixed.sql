-- Database Migration: Add remix metadata and denormalized counts (FIXED)
-- Safe, idempotent migration for remix tracking and metadata

-- 1. Add parent_id column to track remix relationships (standardized name)
DO $$
BEGIN
  -- Check if we need to add parent_id column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='media_assets' AND column_name='parent_id') THEN
    -- Check if parent_asset_id exists and rename it
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name='media_assets' AND column_name='parent_asset_id') THEN
      ALTER TABLE public.media_assets RENAME COLUMN parent_asset_id TO parent_id;
      RAISE NOTICE 'Renamed parent_asset_id to parent_id';
    ELSE
      -- Add new parent_id column
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    RAISE NOTICE 'Added remix_count column';
  END IF;
END $$;

-- 3. Ensure metadata column exists (should be from previous migrations)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='media_assets' AND column_name='metadata') THEN
    ALTER TABLE public.media_assets ADD COLUMN metadata JSONB DEFAULT '{}'::jsonb;
...
-- 10. Add helpful indexes for the new metadata queries (BTREE for text fields)
-- Use BTREE indexes for text fields, not GIN
CREATE INDEX IF NOT EXISTS media_assets_metadata_preset_id_idx
  ON public.media_assets USING BTREE ((metadata->>'presetId'));

CREATE INDEX IF NOT EXISTS media_assets_metadata_mode_idx
  ON public.media_assets USING BTREE ((metadata->>'mode'));

CREATE INDEX IF NOT EXISTS media_assets_metadata_group_idx
  ON public.media_assets USING BTREE ((metadata->>'group'));

-- 11. Create composite index for common queries
CREATE INDEX IF NOT EXISTS media_assets_user_created_idx
  ON public.media_assets (user_id, created_at DESC);

-- 12. Create index for public content queries
CREATE INDEX IF NOT EXISTS media_assets_visibility_created_idx
  ON public.media_assets (visibility, created_at DESC)
  WHERE visibility = 'public';

-- 13. Ensure RLS policies allow reading remix relationships
...
DO $$
BEGIN
  RAISE NOTICE 'Remix metadata migration completed successfully!';
  RAISE NOTICE 'Added: parent_id, remix_count columns';
  RAISE NOTICE 'Created: triggers for automatic remix count maintenance';
