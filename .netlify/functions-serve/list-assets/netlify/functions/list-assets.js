...
exports.handler = async (event) => {
  try {
    ...
    const { data, error } = await supabase.from("assets").select("id, url, resource_type, created_at, public_id, width, height, duration, meta").eq("user_id", userId).order("created_at", { ascending: false }).range(from, to);
