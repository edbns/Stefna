...
var handler = async (event, context) => {
  ...
  try {
    ...
    let claims;
    try {
      claims = import_jsonwebtoken.default.verify(token, jwtSecret, { clockTolerance: 5 });
    } catch (jwtError) {
    ...
    console.log("\u{1F510} Auth context:", { uid, email, claims });
    const body = JSON.parse(event.body || "{}");
    console.log("\u{1F4DD} Update profile request:", { uid, body });
    const { data: userData, error: userUpsertError } = await supabase.from("users").upsert({
      id: uid,
    ...
    if (body.username !== void 0 && body.username !== null) {
      if (body.username === "") {
        ...
        const { data: existingUser, error: checkError } = await supabase.from("profiles").select("id").eq("username", body.username).neq("id", uid).single();
    ...
    if (body.avatar_url !== void 0) updateData.avatar_url = body.avatar_url;
    if (body.share_to_feed !== void 0) updateData.share_to_feed = body.share_to_feed;
    if (body.allow_remix !== void 0) {
      updateData.allow_remix = body.share_to_feed ? body.allow_remix : false;
    }
    if (body.onboarding_completed !== void 0) updateData.onboarding_completed = body.onboarding_completed;
    const { data, error } = await supabase.from("profiles").upsert({
      id: uid,
      // This must match users.id (UUID from Netlify auth)
      ...updateData
    }).select().single();
    if (error) {
