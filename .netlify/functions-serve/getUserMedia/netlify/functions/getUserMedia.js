...
// netlify/lib/db.ts
var url = process.env.NETLIFY_DATABASE_URL || process.env.NEON_DATABASE_URL || process.env.DATABASE_URL || "";
if (!url) {
...
async function requireUser(event) {
  ...
  const m2 = String(raw).match(/^Bearer\s+(.+)$/i);
  if (!m2) {
  ...
  try {
    const { payload } = await jwtVerify(m2[1], new TextEncoder().encode(secret), { algorithms: ["HS256"] });
    const id = payload.sub || payload.user_id || payload.userId || payload.uid || payload.id;
    if (!id) {
...
var handler = async (event) => {
  ...
  try {
    ...
    const media = await sql`
      ...
      LIMIT ${limit}
    `;
    const items = (media ?? []).map((m2) => ({
      id: m2.id,
      userId: m2.owner_id,
      type: m2.resource_type === "video" ? "video" : "photo",
      cloudinary_public_id: m2.public_id,
      url: m2.url,
      prompt: m2.prompt ?? null,
      is_public: m2.visibility === "public",
