...
var handler = async (event) => {
  try {
    ...
    let query = supabaseAdmin.from("notifications").select(`
        ...
        read,
        metadata
      `).eq("user_id", userId).order("created_at", { ascending: false }).limit(parseInt(limit)).range(parseInt(offset), parseInt(offset) + parseInt(limit) - 1);
    if (unread_only === "true") {
    ...
    const { data: unreadCountData, error: countError } = await supabaseAdmin.rpc("get_unread_notification_count", { target_user_id: userId });
