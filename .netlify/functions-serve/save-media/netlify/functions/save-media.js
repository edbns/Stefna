...
var save_media_exports = {};
__export(save_media_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(save_media_exports);
...
var handler = async (event, context) => {
  ...
  if (token) {
    try {
      claims = import_jsonwebtoken.default.verify(token, process.env.JWT_SECRET, { clockTolerance: 5 });
      userId = claims.sub || claims.user_id || claims.uid || claims.id || claims.userId;
      console.log("\u{1F510} JWT verified:", { userId, claims });
    } catch (jwtError) {
      console.error("JWT verification failed:", jwtError);
    }
  }
  console.log("\u{1F510} Auth context:", { userId, hasToken: !!token, claims });
  const runId = body.runId || import_crypto.default.randomUUID();
  ...
  for (const url of urls) {
    if (!url) continue;
    try {
      const res = await import_cloudinary.v2.uploader.upload(url, {
        folder,
        resource_type: "auto",
      ...
      uploaded.push({
        ...
        height: res.height,
        folder: res.folder,
        media_type: res.resource_type === "image" ? "image" : res.resource_type === "video" ? "video" : "raw",
        final: res.secure_url,
        // what your feed maps to
        meta: { url }
        // Store the original URL as metadata
      });
    } catch (e) {
      return err("Cloudinary upload failed", 500, { detail: String(e?.message || e) });
    }
  }
  let dbResult = { skipped: true };
  if (userId && SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
    try {
      ...
      const { error: userError } = await supabase.from("users").upsert({
        id: userId,
        email: claims?.email || `user-${userId}@placeholder.com`,
        name: `User ${userId}`,
        tier: "registered",
      ...
      const assetRows = uploaded.map((u) => ({
        user_id: userId,
        // This ties the media to the authenticated user
        run_id: runId,
        preset_id: body.presetId || null,
        public_id: u.cloudinary_public_id,
        url: u.secure_url,
        // This maps to the main URL
        result_url: u.secure_url,
        // Add result_url for generated content
        ...
        allow_remix: false,
        // Default to false, can be updated later
        created_at: (/* @__PURE__ */ new Date()).toISOString(),
        updated_at: (/* @__PURE__ */ new Date()).toISOString(),
        metadata: {
          ...u.meta,
      ...
      const { data, error } = await supabase.from("media_assets").insert(assetRows).select();
