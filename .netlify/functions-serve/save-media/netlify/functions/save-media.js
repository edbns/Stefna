...
var url = process.env.DATABASE_URL || process.env.NETLIFY_DATABASE_URL || "";
if (!url) {
...
var handler = async (event) => {
  ...
  try {
    ...
    const idem = event.headers["x-idempotency-key"] || event.headers["X-Idempotency-Key"] || crypto.randomUUID();
    const body = JSON.parse(event.body || "{}");
    console.log("\u{1F4E5} Save media request body:", body);
    const {
      // Core fields
      prompt,
      media_type = "image",
      ...
      image_url,
      // AIML generated image URL (primary)
      url: url2,
      // Fallback field name for MoodMorph
      secure_url,
      // Additional fallback
      final,
      // Additional fallback
    ...
    const finalUrl = image_url || url2 || secure_url || final;
    if (!finalUrl) {
    ...
    console.log("\u{1F517} Using URL:", finalUrl);
    if (variations && Array.isArray(variations)) {
      console.log("\u{1F3AD} Processing MoodMorph variations:", variations.length);
      const savedItems = [];
    ...
    console.error("\u{1F4A5} Save media error:", error);
