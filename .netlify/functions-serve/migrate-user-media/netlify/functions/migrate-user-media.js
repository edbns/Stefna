...
// netlify/functions/migrate-user-media.js
...
exports.handler = async (event) => {
  try {
    ...
    console.log(`\u{1F504} Starting migration for user: ${userId}`);
    const { data: userMedia, error: fetchError } = await supabase.from("media_assets").select("id, visibility, env, created_at").eq("user_id", userId).in("env", ["dev", "development"]).order("created_at", { ascending: false });
    if (fetchError) {
    ...
    console.log(`\u{1F4CA} Found ${userMedia.length} media items to migrate for user ${userId}`);
    const batchSize = 10;
    const batches = [];
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      console.log(`\u{1F504} Processing batch ${batchIndex + 1}/${batches.length} (${batch.length} items)`);
      for (const media of batch) {
        try {
          const { error: updateError } = await supabase.from("media_assets").update({
            env: "prod",
            updated_at: (/* @__PURE__ */ new Date()).toISOString()
          }).eq("id", media.id).eq("user_id", userId);
          if (updateError) {
    ...
    const { data: verificationData, error: verificationError } = await supabase.from("media_assets").select("id, env, visibility").eq("user_id", userId).in("env", ["prod"]);
...
*/
//# sourceMappingURL=migrate-user-media.js.map
