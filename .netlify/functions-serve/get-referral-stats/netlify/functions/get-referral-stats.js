...
exports.handler = async (event) => {
  try {
    ...
    console.log(`\u{1F4CA} Getting referral stats for user ${userId} in ${APP_ENV}`);
    const supabase = createClient(
      process.env.SUPABASE_URL,
      process.env.SUPABASE_SERVICE_ROLE_KEY
    );
    let { data: referralData, error: referralError } = await supabase.from("user_referrals").select("*").eq("user_id", userId).single();
    if (referralError && referralError.code === "PGRST116") {
      const { data: newReferral, error: createError } = await supabase.from("user_referrals").insert({
        user_id: userId,
        total_invites: 0,
    ...
    const { data: creditData, error: creditError } = await supabase.from("credits_ledger").select("amount").eq("user_id", userId).eq("env", APP_ENV).like("reason", "referral_bonus_%");
    let actualTokensEarned = 0;
    if (!creditError && creditData) {
      actualTokensEarned = creditData.reduce((sum, record) => sum + (record.amount || 0), 0);
    }
    const { data: recentSignups, error: signupsError } = await supabase.from("referral_signups").select("new_user_email, referrer_bonus, created_at").eq("referrer_user_id", userId).eq("env", APP_ENV).order("created_at", { ascending: false }).limit(10);
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        // Use actual credits from ledger
        recentActivity,
