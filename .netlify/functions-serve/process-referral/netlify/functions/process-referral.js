...
var require_auth = __commonJS({
  "netlify/functions/_auth.js"(exports2, module2) {
    ...
    function getUserId(claims) {
      return claims.sub || claims.user_id || claims.uid || claims.id || claims.userId || null;
    }
    function verifyAuth2(event) {
    ...
    module2.exports = { verifyAuth: verifyAuth2 };
  }
});

// netlify/functions/process-referral.js
var { createClient } = require("@supabase/supabase-js");
var { verifyAuth } = require_auth();
exports.handler = async (event) => {
  try {
    ...
    console.log(`\u{1F381} Processing referral in ${APP_ENV} env: ${referrerEmail} -> ${newUserEmail}`);
    const supabase = createClient(
      process.env.SUPABASE_URL,
      process.env.SUPABASE_SERVICE_ROLE_KEY
    );
    const { data: referrer, error: referrerError } = await supabase.from("users").select("id, email").eq("email", referrerEmail).single();
    ...
    let { data: referrerStats, error: statsError } = await supabase.from("user_referrals").select("total_invites, total_tokens_earned").eq("user_id", referrer.id).single();
    if (statsError && statsError.code === "PGRST116") {
      const { data: newStats, error: createError } = await supabase.from("user_referrals").insert({ user_id: referrer.id }).select("total_invites, total_tokens_earned").single();
      if (createError) throw createError;
      referrerStats = newStats;
    } else if (statsError) {
      throw statsError;
    }
    const { data: existingReferral, error: existingError } = await supabase.from("referral_signups").select("id").eq("new_user_id", newUserId).single();
    if (existingReferral) {
    ...
    const { error: referrerCreditError } = await supabase.from("credits_ledger").insert({
      user_id: referrer.id,
    ...
    const { error: newUserCreditError } = await supabase.from("credits_ledger").insert({
      user_id: newUserId,
      amount: newUserBonus,
    ...
    const { error: signupError } = await supabase.from("referral_signups").insert({
...
*/
//# sourceMappingURL=process-referral.js.map
