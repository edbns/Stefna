...
var { requireJWTUser, resp: resp2, handleCORS: handleCORS2, sanitizeDatabaseUrl: sanitizeDatabaseUrl2 } = (init_auth(), __toCommonJS(auth_exports));
var cleanDbUrl = sanitizeDatabaseUrl2(process.env.NETLIFY_DATABASE_URL || "");
if (!cleanDbUrl) {
  throw new Error("NETLIFY_DATABASE_URL environment variable is required");
}
var sql = neon(cleanDbUrl);
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!referrerEmail || !newUserId || !newUserEmail) {
      return resp2(400, { error: "Referrer email, new user ID, and new user email required" });
    }
    ...
    if (!referrerResult || referrerResult.length === 0) {
      ...
      return resp2(404, { error: "Invalid referrer email" });
    }
    ...
    if (existingReferral) {
      ...
      return resp2(400, { error: "User already referred" });
    }
