...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log(`\u{1F50D} Checking tier promotion for user: ${user.userId}`);
    ...
    try {
      ...
      if (promotions && promotions.length > 0) {
        ...
        if (promotionTime > oneHourAgo) {
          console.log(`\u{1F389} User ${user.userId} was recently promoted to ${recentPromotion.to_tier}`);
          return resp2(200, {
            promoted: true,
            newTier: recentPromotion.to_tier,
            oldTier: recentPromotion.from_tier || "registered",
            message: recentPromotion.to_tier === "verified" ? "Congratulations! You've been promoted to Verified Creator! \u{1F389}" : "Amazing! You've reached Contributor status! \u{1F31F}",
    ...
    return resp2(500, { error: e?.message || "Internal server error" });
  }
};
