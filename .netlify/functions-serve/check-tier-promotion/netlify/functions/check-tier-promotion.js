...
// netlify/functions/check-tier-promotion.js
...
exports.handler = async (event) => {
  try {
    ...
    console.log(`\u{1F50D} Checking tier promotion for user: ${userId}`);
    const { data: recentPromotion, error: promotionError } = await supabase.from("user_promotions").select("*").eq("user_id", userId).order("promoted_at", { ascending: false }).limit(1).single();
    ...
    if (recentPromotion) {
      ...
      if (promotionTime > oneHourAgo) {
        console.log(`\u{1F389} User ${userId} was recently promoted to ${recentPromotion.new_tier}`);
        return {
          statusCode: 200,
          ...
          body: JSON.stringify({
            ...
            message: recentPromotion.new_tier === "verified" ? "Congratulations! You've been promoted to Verified Creator! \u{1F389}" : "Amazing! You've reached Contributor status! \u{1F31F}",
...
*/
//# sourceMappingURL=check-tier-promotion.js.map
