...
var { resp: resp2, handleCORS: handleCORS2, sanitizeDatabaseUrl: sanitizeDatabaseUrl2 } = (init_auth(), __toCommonJS(auth_exports));
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!email || !newTier) {
      return resp2(400, { error: "Email and newTier are required" });
    }
    const validTiers = ["registered", "verified", "contributor"];
    if (!validTiers.includes(newTier)) {
      return resp2(400, { error: "Invalid tier. Must be one of: registered, verified, contributor" });
    }
    ...
    if (!userResult || userResult.length === 0) {
      return resp2(404, { error: "User not found" });
    }
    ...
    if (!updateResult || updateResult.length === 0) {
      return resp2(500, { error: "Failed to update user tier" });
    }
