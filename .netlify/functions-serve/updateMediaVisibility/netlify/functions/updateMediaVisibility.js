...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (updateData.visibility === "public") {
      const { data: assetRow, error: fetchErr } = await supabase.from("media_assets").select("id, user_id, prompt").eq("id", assetId).eq("user_id", userId).single();
    ...
    const { data: updated, error } = await supabase.from("media_assets").update(updateData).eq("id", assetId).eq("user_id", userId).select().single();
    if (error) {
