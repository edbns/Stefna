...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!user) {
      return resp2(401, { error: "Unauthorized" });
    }
    const body = JSON.parse(event.body || "{}");
    const { assetId, visibility, allowRemix } = body;
    if (!assetId) {
      return resp2(400, { error: "assetId is required" });
    }
    if (visibility && !["private", "public"].includes(visibility)) {
      return resp2(400, { error: 'visibility must be "private" or "public"' });
    }
    if (typeof allowRemix !== "undefined" && typeof allowRemix !== "boolean") {
      return resp2(400, { error: "allowRemix must be a boolean" });
    }
    const updateData = {};
    if (visibility !== void 0) updateData.visibility = visibility;
    if (allowRemix !== void 0) updateData.allow_remix = allowRemix;
    if (Object.keys(updateData).length === 0) {
      return resp2(400, { error: "No valid fields to update" });
    }
    if (updateData.visibility === "public") {
      try {
        ...
        if (!assetRows || assetRows.length === 0) {
          return resp2(404, { error: "Media asset not found or access denied" });
        }
        ...
        if (BLOCK_LIST.some((k) => prompt.includes(k))) {
          return resp2(400, { error: "Content blocked by moderation. Please revise your prompt before sharing publicly." });
        }
      } catch (fetchErr) {
    ...
    try {
      const updatedRows = await sql`
        UPDATE media_assets 
        SET 
          ${updateData.visibility !== void 0 ? sql`visibility = ${updateData.visibility}` : sql``},
          ${updateData.allow_remix !== void 0 ? sql`allow_remix = ${updateData.allow_remix}` : sql``},
          updated_at = NOW()
        WHERE id = ${assetId} AND owner_id = ${user.userId}
      ...
      if (updateErr.message.includes("Cannot allow remix on private media")) {
        return resp2(400, { error: "Cannot allow remix on private media. Make it public first." });
      }
      if (updateErr.message.includes("Cannot remix")) {
        return resp2(400, { error: updateErr.message });
      }
      return resp2(400, { error: updateErr.message || "Update failed" });
    }
  } catch (error) {
    console.error("updateMediaVisibility error:", error);
    return resp2(500, { error: "Internal server error" });
  }
};
...
*/
//# sourceMappingURL=updateMediaVisibility.js.map
