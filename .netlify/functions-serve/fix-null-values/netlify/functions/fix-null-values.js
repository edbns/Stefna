...
// netlify/functions/fix-null-values.js
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log(`\u{1F527} Fixing null values for user ${userId}, setting env to: ${env}`);
    const { data: nullImages, error: findError } = await supabase.from("media_assets").select("id, env, visibility, created_at").eq("user_id", userId).or("env.is.null,visibility.is.null");
    if (findError) {
    ...
    console.log(`\u{1F4CA} Found ${nullImages.length} images with null values to fix`);
    const { data: fixed, error: fixError } = await supabase.from("media_assets").update({
      env,
      visibility: "private",
      // Safer default - user can then choose to share
      updated_at: (/* @__PURE__ */ new Date()).toISOString()
    }).eq("user_id", userId).or("env.is.null,visibility.is.null").select("id, env, visibility, updated_at");
    if (fixError) {
...
*/
//# sourceMappingURL=fix-null-values.js.map
