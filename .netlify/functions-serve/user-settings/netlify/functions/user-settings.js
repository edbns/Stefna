...
exports.handler = async (event) => {
  try {
    ...
    if (event.httpMethod === "GET") {
      console.log(`\u{1F4E5} Getting settings for user: ${userId}`);
      const { data: settings, error } = await supabase.from("user_settings").select("share_to_feed, allow_remix, updated_at").eq("user_id", userId).single();
    ...
    if (event.httpMethod === "POST") {
      ...
      console.log(`\u{1F4DD} Updating settings for user ${userId}:`, { shareToFeed, allowRemix });
      const { data: updated, error } = await supabase.from("user_settings").upsert({
        user_id: userId,
        ...
        onConflict: "user_id"
      }).select("share_to_feed, allow_remix, updated_at").single();
      if (error) {
