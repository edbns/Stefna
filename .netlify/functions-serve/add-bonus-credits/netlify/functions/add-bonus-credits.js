...
// netlify/functions/add-bonus-credits.js
...
exports.handler = async (event) => {
  try {
    ...
    console.log(`\u{1F4B0} Adding ${amount} bonus credits to user ${userId} in ${APP_ENV}`);
    const supabase = createClient(
      process.env.SUPABASE_URL,
      process.env.SUPABASE_SERVICE_ROLE_KEY
    );
    const { error: creditError } = await supabase.from("credits_ledger").insert({
      user_id: userId,
      amount,
...
*/
//# sourceMappingURL=add-bonus-credits.js.map
