...
exports.handler = async (event) => {
  try {
    ...
    console.log(`\u{1F30D} getQuota using env: ${APP_ENV} for user: ${userId}`);
    const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
    const startUTC = /* @__PURE__ */ new Date();
    startUTC.setUTCHours(0, 0, 0, 0);
    const { data: creditRows, error: creditError } = await supa.from("credits_ledger").select("amount").eq("user_id", userId).eq("env", APP_ENV).gte("created_at", startUTC.toISOString());
    if (creditError) {
    ...
    const { data: weeklyRows, error: weeklyError } = await supa.from("credits_ledger").select("amount").eq("user_id", userId).eq("env", APP_ENV).gte("created_at", weekAgo.toISOString());
    const weekly_used = weeklyError ? 0 : (weeklyRows || []).reduce((sum, row) => sum + (row.amount || 0), 0);
    console.log(`\u{1F4CA} Quota for user ${userId} (${APP_ENV}): daily ${daily_used}/${daily_limit}, weekly ${weekly_used}/${weekly_limit}`);
    return {
      statusCode: 200,
      headers: { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    const body = status === 200 ? { daily_used: 0, daily_limit: 30, weekly_used: 0, weekly_limit: 150 } : { error: e.message || "Unauthorized" };
...
*/
//# sourceMappingURL=getQuota.js.map
