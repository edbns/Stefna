...
var normalizeTyp, checkAudiencePresence;
...
__export(db_exports, {
  json: () => json,
  parseUserIdFromJWT: () => parseUserIdFromJWT,
  sql: () => sql
});
var url, sql, json, parseUserIdFromJWT;
var init_db = __esm({
  "netlify/functions/_db.ts"() {
    "use strict";
    init_serverless();
    ...
    parseUserIdFromJWT = async (authHeader) => {
...
function sanitizeDatabaseUrl(url2) {
  if (!url2) return "";
  try {
...
var { sql: sql2 } = (init_db(), __toCommonJS(db_exports));
var { requireAuth: requireAuth2, httpErr: httpErr2 } = (init_auth(), __toCommonJS(auth_exports));
exports.handler = async (event) => {
  ...
  try {
    const { userId } = requireAuth2(event.headers.authorization);
    console.log("\u2705 User authenticated:", userId);
    let profile = await sql2`
    ...
    profile = profile[0];
    if (!profile) {
      console.log("\u{1F4DD} Creating new profile for user:", userId);
      try {
