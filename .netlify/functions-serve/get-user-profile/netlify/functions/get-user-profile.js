...
exports.handler = async (event) => {
  try {
    ...
    console.log(`\u{1F4E5} Getting user profile for: ${userId}`);
    const { data: user, error: userError } = await supabase.from("users").select("id, name, email, tier").eq("id", userId).single();
    if (userError) {
      console.error("\u274C User not found in users table:", userError);
      return { statusCode: 401, body: JSON.stringify({ error: "User not found" }) };
    }
    const { data: profile, error } = await supabase.from("profiles").select("id, username, avatar_url, share_to_feed, allow_remix, onboarding_completed, created_at, updated_at").eq("id", userId).single();
