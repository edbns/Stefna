...
exports.handler = async (event) => {
  try {
    ...
    console.log(`\u{1F4DD} Updating user ${userId}:`, { name, avatar });
    const updates = {};
    if (name) updates.name = name;
    if (avatar) updates.avatar_url = avatar;
    const { data: updated, error } = await supabase.from("users").update(updates).eq("id", userId).select("id, name, avatar_url, updated_at").single();
    if (error) {
