...
// netlify/functions/bulk-share.js
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log(`\u{1F504} Bulk share requested by user ${userId} for env: ${env}`);
    const { data: mediaToUpdate, error: countError } = await supabase.from("media_assets").select("id, visibility, env").eq("user_id", userId).or(`visibility.neq.public,env.neq.${env}`);
    if (countError) {
    ...
    console.log(`\u{1F4CA} Found ${mediaToUpdate.length} items to update`);
    const { data: updated, error: updateError } = await supabase.from("media_assets").update({
      visibility: "public",
      env,
      updated_at: (/* @__PURE__ */ new Date()).toISOString()
    }).eq("user_id", userId).or(`visibility.neq.public,env.neq.${env}`).select("id, visibility, env, updated_at");
    if (updateError) {
...
*/
//# sourceMappingURL=bulk-share.js.map
