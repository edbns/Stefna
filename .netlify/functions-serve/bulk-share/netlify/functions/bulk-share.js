...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!user) {
      return resp2(401, { error: "Authentication required" });
    }
    const body = JSON.parse(event.body || "{}");
    const { action, env = "prod" } = body;
    if (action !== "bulk-share") {
      return resp2(400, { error: "Invalid action" });
    }
    console.log(`\u{1F504} Bulk share requested by user ${user.userId} for env: ${env}`);
    ...
    console.log(`\u{1F4CA} Found ${countResult.length} items to update`);
